...
*/
  ...
  object DefTrans {
    def unapply[T](e: Exp[T]): Option[Def[T]] = Def.unapply(apply(e))
  }
  override def transformStm(stm: Stm): Exp[Any] = stm match {
    case TP(lhs, rhs) => rhs match {
      case Reflect(RandIfThenElse(f@DefTrans(Reflect(Flip(p), _, _)), a@Block(DefTrans(Always(Const(1)))), b@Block(DefTrans(Always(Const(0))))), u, es) if defUse(lhs)==1 && defUse(f)==1 =>
        bernoulliRewrites += 1
        reflectEffect(Bernoulli(p, 1)/*.asInstanceOf[Def[A]]*/, Alloc())(mtype(lhs.tp), implicitly[SourceContext])
      case Reflect(RandPlus(sx@DefTrans(Reflect(Bernoulli(p1, n1), u1, es1)), sy@DefTrans(Reflect(Bernoulli(p2, n2), u2, es2))), u, es) if defUse(lhs)==1 && defUse(sx)==1 && defUse(sy)==1 && p1==p2 =>
        bernoulliAdditions += 1
        reflectEffect(Bernoulli(p1, n1+n2)/*.asInstanceOf[Def[A]]*/, Alloc())(mtype(lhs.tp), implicitly[SourceContext])
      case RandPlus(x@DefTrans(Always(Const(0))), sy) =>
        littleThings += 1
...
trait DeepLang extends DeepLangExp with CompileScala { q =>
  ...
  object codegen extends ScalaGenDeepLang {
    ...
    private def defUse(sym: Sym[_]): Int = defUseMap.getOrElse(sym, 0)
    def emitSym(sym: Sym[_]): Boolean = {
      sym match {
